 ...
 */
export async function prepareDrink(order) {
    log.info('Preparing drink', { orderId: order.orderId, drinkType: order.drinkType, size: order.size });
    // Simulate preparation time (in a real system, the actual work is
    // the barista making the drink — this activity just records the step)
    return {
        orderId: order.orderId,
        status: 'prepared',
        timestamp: new Date().toISOString(),
...
//# sourceMappingURL=barista.js.map